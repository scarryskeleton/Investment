...
# ui.explore / ui.research / ui.practice read the account currency as
# C.ACCOUNT_CCY (module-qualified) rather than importing the name directly,
# since — unlike EUR/EUR2, which are only ever mutated in place — this one is
# reassigned each rerun and a plain `from ui.common import ACCOUNT_CCY` would
# freeze a stale copy at import time.
...
if run:
    ...
    if mode == "shares" and latest:
        ...
        latest = {t: v * C._fx_spot(_meta.get(t, {}).get("currency") or ACCOUNT_CCY,
                                    ACCOUNT_CCY)
